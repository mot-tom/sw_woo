/******************************************************************************
    ...
    --- Extended Stillinger-Weber Potential for Si, O, Ni Mixed Systems ---

         Copyright (c) Takanobu Watanabe 2012  All Rights Reserved
******************************************************************************/
#include "mddef.h"
...
static ESWNI2Body * get_ESWNI_2body(AtomIonType, AtomIonType);
static ESWNI3Body * get_ESWNI_3body(AtomIonType, AtomIonType, AtomIonType);
...
double set_ESWNI_scut(void)
{
  ...
  for(i=0;i<MAXELEMENT;i++){
    for(j=i;j<MAXELEMENT;j++){
      tmpcut = get_EVSR_scutoff(AtomIonType(i), AtomIonType(j));
...
double get_ESWNI_scutoff(AtomIonType a1, AtomIonType a2)
{
  double cut = 0.0;
  int i;
  ESWNI2Body *param2;
  ...
  if(a1 == Si && a2 == Si) return 3.9258;   ////
  if(a1 == Si && a2 == O ) return 3.80; ////
  if(a1 == O  && a2 == Si) return 3.80; ////
  if(a1 == O  && a2 == O ) return 2.30;
  if(a1 == Ni && a2 == Ni) return 3.80; ////
  if(a1 == Ni && a2 == O ) return 3.0;  ////
  if(a1 == O  && a2 == Ni) return 3.0;  ////
  if(a1 == Ni && a2 == Si) return 3.30; ////
  if(a1 == Si && a2 == Ni) return 3.30; ////


  param2 = get_ESWNI_2body(a1, a2);
  if(cut < param2->a) cut = param2->a;
  for(i=0;i<MAXELEMENT;i++){
    param3 = get_ESWNI_3body(a1, a2, (AtomIonType)i);
    if(cut < param3->a1ij) cut = param3->a1ij;
    if(cut < param3->a2ik) cut = param3->a2ik;
    param3 = get_ESWNI_3body(a2, a1, (AtomIonType)i);
    if(cut < param3->a1ij) cut = param3->a1ij;
    if(cut < param3->a2ik) cut = param3->a2ik;
  }
...
static ESWNI2Body * get_ESWNI_2body(AtomIonType ai, AtomIonType aj)
{
  static ESWNI2Body prm;
...
static ESWNI3Body * get_ESWNI_3body(AtomIonType aj,
                                    AtomIonType ai,
                                    AtomIonType ak)
{
  static ESWNI3Body prm;
...
void calc_ESWNI_force(void)
{
  ...
  ESWNI2Body * prm2; ESWNI3Body * prm3;
  double tmp[11], pe;
  ...
  for(i=0;i<N;i++){
    ...
    for(i1=1;i1<=neighbor;++i1){  // neighbor list is from 1 to "neighbor"
      ...
      if(i < j){
        ...
        if((ai == Si && aj == Si) || (ai == O  && aj == O) || ai == Ni
           || aj == Ni){
          prm2 = get_ESWNI_2body(ai, aj);
          a = prm2->a;
          if(rij < a){
            A = prm2->A;
            B = prm2->B;
